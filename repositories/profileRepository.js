...
class ProfileRepository {
  ...
  async findInstructorByAccountId(accountId) {
    const db = await connectDB();
    const [rows] = await db.query("SELECT * FROM instructor WHERE AccID = ?", [
      accountId,
    ]);
  ...
  async findLearnerByAccountId(accountId) {
    const db = await connectDB();
    const [rows] = await db.query("SELECT * FROM learner WHERE AccID = ?", [
      accountId,
    ]);
    return rows[0] || null; // Trả về null nếu không tìm thấy
  }
  async findAdminByAccountId(accountId) {
    const db = await connectDB();
    const [rows] = await db.query("SELECT * FROM admin WHERE AccID = ?", [
      accountId,
    ]);
    return rows[0] || null; // Trả về null nếu không tìm thấy
  }
  ...
  async updateInstructorAvatar(accountId, avatarUrl) {
    const db = await connectDB();
    await db.query("UPDATE instructor SET ProfilePicture = ? WHERE AccID = ?", [
      avatarUrl,
      accountId,
    ]);
  }
  ...
  async updateLearnerAvatar(accountId, avatarUrl) {
    const db = await connectDB();
    await db.query("UPDATE learner SET ProfilePicture = ? WHERE AccID = ?", [
      avatarUrl,
      accountId,
    ]);
  }
  ...
  async updateParentAvatar(accountId, avatarUrl) {
    const db = await connectDB();
    await db.query("UPDATE parent SET ProfilePicture = ? WHERE AccID = ?", [
      avatarUrl,
      accountId,
    ]);
  }
