...
class LearnerAssignmentService {
  async getAssignmentDetails(assignmentId, accountId) {
    ...
    const assignment = await learnerassignmentRepository.getAssignmentById(
      assignmentId,
      learner.LearnerID
    );
    if (!assignment) {
    ...
    const isEnrolled = await courseRepository.checkEnrollment(
      assignment.CourseID,
      learner.LearnerID
    );
    if (!isEnrolled) {
  ...
  async getAssignmentQuestions(assignmentId, accountId) {
    try {
      ...
      const questions =
        await learnerassignmentRepository.getAssignmentQuestions(assignmentId);
  ...
  async submitAssignment(assignmentId, accountId, submissionData) {
    try {
      ...
      const existingSubmission =
        await learnerassignmentRepository.getSubmissionByAssignmentAndLearner(
          assignmentId,
          learner.LearnerID
        );
      ...
      if (
        ...
        Object.keys(submissionData.answers).length > 0
      ) {
        console.log("Processing answers:", submissionData.answers); // DEBUG

        // Chuyển đổi answers từ object sang array
        answers = Object.entries(submissionData.answers).map(
          ([assignmentQuestionId, answer]) => ({
        ...
        submissionScore = await this.handleQuizSubmission(
          assignmentId,
          answers,
          learner.LearnerID
        );
  ...
  async handleQuizSubmission(assignmentId, answers, learnerId) {
    ...
    for (const answer of answers) {
      ...
      if (!question) {
        console.log(
          "Question not found for AssignmentQuestionId:",
      ...
      const isCorrect = await this.checkAnswerCorrectness(
        question,
        answer.answer
      );
  ...
  async checkAnswerCorrectness(question, userAnswer) {
    ...
    switch (question.Type) {
      case "multiple_choice":
        ...
        console.log(
          "Multiple choice - Correct option:",
          correctOption?.OptionID
        ); // DEBUG
        return (
          correctOption && userAnswer === correctOption.OptionID.toString()
        );
  ...
  checkMatchingAnswer(question, userAnswer) {
    try {
      ...
      // So sánh các cặp
      const userSorted = JSON.stringify(
        userPairs.sort((a, b) => a.left.localeCompare(b.left))
      );
      const correctSorted = JSON.stringify(
        correctPairs.sort((a, b) => a.left.localeCompare(b.left))
      );
  ...
  checkMatchingAnswer(question, userAnswer) {
    try {
      ...
      for (const key of correctKeys) {
        ...
        const correctValue = correctObj[key];

        if (userValue !== correctValue) {
          console.log(
            `Mismatch at key [${key}]: User says [${userValue}], Correct is [${correctValue}]`
          );
          return false;
        }
      }
      ...
      console.error("Matching answer validation error:", error);
      return false;
    }
  }
  ...
  async getSubmissionDetails(submissionId, accountId) {
    ...
    const submission = await learnerassignmentRepository.getSubmissionById(
      submissionId,
      learner.LearnerID
    );
    if (!submission) {
    ...
    submission.assets = await learnerassignmentRepository.getSubmissionAssets(
      submissionId
    );
    ...
    if (submission.Assignment.Type === "quiz") {
      submission.answers =
        await learnerassignmentRepository.getSubmissionAnswers(
          submissionId,
          learner.LearnerID
        );
    }
  ...
  async getAssignmentResults(assignmentId, accountId) {
    ...
    if (results.canShowAnswers && assignment.Type === "quiz") {
      results.questions =
        await learnerassignmentRepository.getAssignmentQuestionsWithCorrectAnswers(
          assignmentId
        );
      results.userAnswers =
        await learnerassignmentRepository.getSubmissionAnswers(
